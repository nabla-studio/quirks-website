...
<TypeTable
  type={{
    ...
    chains: {
      description:
        "Array of chains that you intend to use, imported from @nabla-studio/chain-registry.",
      type: "Chain[]",
      default: "[]",
    },
    assetsLists: {
    ...
    walletConnectOptions: {
      description:
        "Required if you are using Wallet Connect; it is used to define the parameters you request from Wallet Connect, such as the Project ID.",
...
<TypeTable
  type={{
    providerOpts: {
      description:
        "It is based on Wallet Connect's universal provider configuration and allows us to define our app's metadata and customise some behaviours.",
